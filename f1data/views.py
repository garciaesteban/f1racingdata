...
class ConstructorStandingsRedirectView(RedirectView):
    def get_redirect_url(self,*args,**kwargs):
        year = Season.objects.first().year
        round = DriverStanding.objects.order_by("race").first().race.round
        return f"{year}/"

class ConstructorStandings(ListView):
    template_name = "f1data/constructor_standings/constructor_standings.html"
    context_object_name = 'constructor_standings'
    ...
    def get_queryset(self,**kwargs):
        ...
        for race in races:
            query_set = ConstructorStanding.objects.filter(race=race).order_by('position')
            if query_set.exists():
                constructor_standings.append(query_set)
    ...
    def get_context_data(self,**kwargs):
        ...
        for race in races:
            if ConstructorStanding.objects.filter(race=race).exists():
                rounds.append(race)
